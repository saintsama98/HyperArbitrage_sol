...
async function getAmountOut(routerAddress: string, amountIn: bigint, path: string[]): Promise<bigint> {
...
async function main() {
  ...
  const bot = new ethers.Contract(contractAddress, abi, wallet);

  console.log("Connected to contract at:", contractAddress);

  // 1. Set amount to borrow (e.g., 1 WETH)
  const amountIn = ethers.parseUnits("1", 18); // Adjust decimals for TOKEN_A
  ...
  console.log(`Simulated profit: ${ethers.formatUnits(profit, 18)} TOKEN_A`);
